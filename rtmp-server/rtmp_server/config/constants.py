...
Bash/systemd-файлы в debian/ не генерируются из этого модуля автоматически
(отдельного шага рендеринга нет) — значения там дублируются вручную и
сверяются с этим файлом тестом tests/test_unit_consistency.py, который
падает при расхождении. При добавлении/переносе порта или пути правь
сначала здесь, потом debian/, и прогоняй тесты.
"""
