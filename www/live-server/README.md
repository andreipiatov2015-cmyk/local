...
## Таблицы заявок
- Раздел таблиц работает внутри админ-страницы (`/admin?section=tables`) в том же Flask-процессе.
- `/tables` делает redirect на `/admin?section=tables`.
- Отдельный запуск `uvicorn`/FastAPI не требуется.
- API таблиц доступно по путям `/api/tables*` на том же домене и порту.
- Используется общая авторизация сайта (без отдельного email-кода для `/tables`).
